...
contract TokenSales {
  ...
  function setForSale(uint256 _tokenId, uint256 _price) public {
    ...
    require(nftAddress.isApprovedForAll(tokenOwner, address(this)), "Token owner dide not approve TokenSales contrace");
    tokenPrice[_tokenId] = _price;
    }
  function purchanseToken(uint256 _tokenId) public payable {
    uint256 price = tokenPrice[_tokenId];
    address tokenSeller = nftAddress.ownerOf(_tokenId);
    require(msg.value >= price, "caller sent klay lower than price");
    require(msg.sender != tokenSeller, "caller is TokenSeller");
