...
const App = {
  ...
  checkValidKeystore: function (keystore) {
    ...
    console.log(parsedKeystore.keyring)
    console.log('------------------')
    console.log(parsedKeystore.crypto)
    const isValidKeystore = parsedKeystore.keyring || parsedKeystore.crypto;
  ...
  checkTokenExists: async function () {   
    ...
    if (result) {
      $('#t-message').text('이미 토큰화 된 썸네일 입니다.');
    } else {
  ...
  mintYTT: async function (videoId, author, dateCreated, hash) {
    ...
    const feePayer = cav.klay.accounts.wallet.add('0xd479b0a840dca58dac91d33af7733667676d8bf5dedb5e863f1fe769b0c5b6e5')
    ...
    if (balance === 0) {
      ...
      for (var i = 0; i < balance; i++) {
        (async () => {
          ...
          if (parseInt(price) > 0) 
            this.renderMyTokensSale(tokenId, ytt, metadata, isApproved);
        })();''
      }
    }
  },   
  ...
  displayAllTokens: async function (walletInstance) {   
    ...
    if (totalSupply === 0) {
      $('#allTokens').text("현재 발행된 토큰이 없습니ㅏ.");
    } else {
  ...
  sellToken: async function (button) {    
    ...
    try {
      ...
      const { rawTransaction: senderRawTransaction } = await cav.klay.accounts.signTransaction({
        ...
        data: tsContract.methods.setForsale(tokenId, cav.utils.toPeb(amout, 'KLAY')).encodeABI(),
        gas:  '500000',
        value: cav.utils.toPeb('0', 'KLAY'),
      }, sender.privateKey)
  ...
  getBasicTemplate: function(template, tokenId, ytt, metadata) {  
    ...
    templametadata.properties.description.descriptionfind('img').attr('src',metadata.properties.image.description)
